...
session_start();
 ...
 if($_POST)
 {
   ...
   $goals=html_entity_decode($goals);
   //echo strip_tags($goals);
  // echo $goals;
...
{
  $query = "UPDATE members SET ProfilePicture = :profilePicture WHERE UserID = :userID";
 ...
 // $query = "UPDATE members SET Story = :story WHERE UserID = :id ";
 // $statement = $db->prepare($query);
 //
 ...
 $query = "UPDATE goals SET GoalTitle = :goalTitle,GoalDescription = :goalDescription WHERE UserID =:userID";
  $statement = $db->prepare($query);
 $statement ->bindValue(":userID",$_SESSION['userID']);
